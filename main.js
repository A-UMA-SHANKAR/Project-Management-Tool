document.addEventListener("DOMContentLoaded", function () {
    ...
    const allowedNames = ["Hara", "Chandu", "Ali", "Anshika", "Satya"];
    ...
    form.addEventListener("submit", function (event) {
        ...
        newTask.innerHTML = `<style>input,select,textarea{
            border-radius: 5px;
        ...
        textarea{
            ...
            <div class="comment-section">
                <textarea class="comment-input" placeholder="Add a comment"></textarea><br>
            ...
            <div class="github-link">
                ...
                <input style="width:250px;"type="text" name="githubLink" class="github-link-input"><br>
            </div>
        ...
        completeTaskButton.addEventListener("click", function () {
            ...
            if (githubLink) {
                ...
                // Update the GitHub link in the completed task
                const githubLinkElement = completedTask.querySelector(".github-link");
                githubLinkElement.innerHTML = `<p class="task-github-link">GitHub Link: <a href="${githubLink}" target="_blank">${githubLink}</a></p>`;
        
                // Move completed task to the completed tasks list
                completedTasksList.appendChild(completedTask);
        
                // Remove from My Tasks and Assigned Tasks lists
                myTasksList.removeChild(newTask);
                assignedTasksList.removeChild(newTask);
        
                // Remove start and complete buttons from the cloned task
                const startButton = newTask.querySelector(".btn-start");
                const completeButton = newTask.querySelector(".btn-complete");
                startButton && startButton.remove();
                completeButton && completeButton.remove();
            } else {
